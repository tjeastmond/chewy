...
describe('resume exports', () => {
  ...
  test('exports JSON/YAML/CSV/TXT and renders HTML', async () => {
    ...
    expect(html).toMatch(/@media print[\s\S]*h1 \{ font-size: 40px;/)
    expect(html).toMatch(/@media print[\s\S]*\.job-title \{ font-size: 15px;/)
    expect(html).toMatch(/@media print[\s\S]*\.job-meta \{ font-size: 11px;/)
    expect(html).toMatch(/@media print[\s\S]*li \{ font-size: 12px;/)
